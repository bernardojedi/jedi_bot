...
BotServer.listen("runeCombo", function(name, message)
  if storage[BotPanelName].runeCombo then
    local runeId = message["runeId"]
    local targetId = message["targetId"]
    if runeId and targetId then
      local creature = getCreatureById(targetId)
      if creature then
        g_game.useInventoryItemWith(runeId, creature, 0)
      end
    end
  end
end)
...
onKeyPress(function(keys)
  if keys == storage[BotPanelName].comboSDhotkey then
    ...
    if creature then
      BotServer.send("runeCombo", {runeId=3155, targetId=creature:getId()})
    end
  end
end)
