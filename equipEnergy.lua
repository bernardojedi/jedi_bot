...
local healthp_for_energy = 40;
local healthp_for_original = 80;
local manap_for_original = 5;

macro(100, "Auto E-ring", function()
    
    local manaPercent = math.floor(100 * (player:getMana() / player:getMaxMana()))
    if (manaPercent <= manap_for_original and getFinger() and getFinger():getId() == energy_ring_equiped) then
    ...
    elseif (player:getHealthPercent() >= healthp_for_original and (not getFinger() or getFinger():getId() == energy_ring_equiped)) then
        local ring = findItem(original_ring:getId())
        g_game.move(ring, {x=65535, y=9, z=0}, 1)
    end
end)